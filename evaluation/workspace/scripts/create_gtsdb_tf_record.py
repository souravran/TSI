...
from TensorflowModelZoo.research.object_detection.utils import dataset_util
from TensorflowModelZoo.research.object_detection.utils import label_map_util
...
def main(_):
  ...
  label_map_dict = label_map_util.get_label_map_dict(FLAGS.label_map_path, use_display_name=False)
  rev_label_map_dict = dict((v,k) for k,v in label_map_dict.items())
  print(rev_label_map_dict)

  logging.info('Reading from GTSDB dataset.')
  image_dir = data_dir
  examples_gt_path = os.path.join(data_dir, 'gt.txt')
  examples_list = ['%05d.jpg' % x for x in range(926)]

  # 600 training examples and 326 are validation examples
  train_examples = examples_list[:600] + examples_list[900:915] + examples_list[920:]
  val_examples = examples_list[600:900] + examples_list[915:920]
  print('Train examples: {}, Validation examples: {}'.format(len(train_examples), len(val_examples)))
  ...
  val_output_path = os.path.join(FLAGS.output_dir, 'gtsdb_validation.record')
  create_tf_record(train_output_path, rev_label_map_dict, examples_gt_path,
                   image_dir, train_examples)
  create_tf_record(val_output_path, rev_label_map_dict, examples_gt_path,
